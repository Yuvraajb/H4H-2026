...
//  Survivor MVP
//
//  Emergency assistance tablet UI: camera as main focus, four side panels
//  (Guidance/images, Chat, Menu, Misc). Glass-style panels, minimal camera obstruction.
//

import SwiftUI

// MARK: - Design tokens (match reference: dark glass, primary blue)
private let primaryBlue = Color(red: 0.145, green: 0.388, blue: 0.922)   // #2563EB
private let primaryDark = Color(red: 0.114, green: 0.306, blue: 0.847)  // #1D4ED8
...
struct ContentView: View {
    ...
    private var mainContent: some View {
        ZStack {
            ...
            VStack(alignment: .leading, spacing: 0) {
                liveFeedBadge
                // Visible label so you can confirm this is the new 4-panel UI
                Text("Aura Emergency")
            ...
            HStack(alignment: .top, spacing: 0) {
                Spacer(minLength: 0)
                rightPanelsStack(orbModel: voiceOrbModel)
                    .frame(width: 352)
                    .padding(.trailing, 24)
                    .padding(.vertical, 24)
            }
    ...
    private var liveFeedBadge: some View {
        HStack(spacing: 8) {
            Circle()
                ...
                .overlay(
                    Circle().fill(Color.red.opacity(0.6)).scaleEffect(1.4)
                )
    ...
    @ViewBuilder
    private func rightPanelsStack(orbModel: VoiceOrbModel) -> some View {
        VStack(alignment: .trailing, spacing: 16) {
            // AURA AI status pill
            auraStatusPill
            // Chat (Assistant) panel — takes remaining vertical space
            AssistantChatPanelView(orbModel: orbModel)
                .frame(maxHeight: .infinity)
            // Menu panel (scenario, session actions)
            MenuPanelView()
            // Misc panel (Alert EMS, Light, Siren)
            MiscActionsPanelView()
        }
    }

    private var auraStatusPill: some View {
        HStack(spacing: 12) {
            HStack(spacing: 6) {
                Circle()
                    .fill(primaryBlue)
    ...
    private var cameraPane: some View {
        ZStack {
            ...
            // Optional minimal toggles (top-left, below LIVE FEED) — keep small
...
// MARK: - Glass panel style (dark blur, border, shadow — reference HTML)
...
// MARK: - Left panel: Guidance (images / step-by-step) — scrollable, backend-driven, voice-nav
struct GuidancePanelView: View {
    @Environment(CrisisCopilotModel.self) private var model

    private var steps: [ChatStep] {
        let latest = model.latestGuidanceSteps
        if latest.isEmpty {
            return [
                ChatStep(
                    instruction: "Tap the shoulder and shout \"Are you okay?\". Look for chest movement indicating breathing.",
                    imageURL: nil
                )
            ]
        }
    ...
    var body: some View {
        VStack(spacing: 0) {
            // Header
            HStack {
                Label("1. GUIDANCE", systemImage: "cross.case.fill")
                    .font(.system(size: 12, weight: .bold))
                    .foregroundStyle(.white)
                Spacer()
                Text("\(steps.count) step\(steps.count == 1 ? "" : "s")")
                    .font(.system(size: 10, weight: .medium))
                    .foregroundStyle(primaryBlue.opacity(0.9))
                    .padding(.horizontal, 8)
                    .padding(.vertical, 4)
                    .background(primaryBlue.opacity(0.2), in: RoundedRectangle(cornerRadius: 6))
            }
            .padding(16)
            ...
            // Scrollable list of steps (backend images shown via AsyncImage)
            ScrollViewReader { proxy in
                ScrollView {
                    LazyVStack(alignment: .leading, spacing: 20) {
                        ForEach(Array(steps.enumerated()), id: \.element.id) { index, step in
                            guidanceStepCard(step: step, number: index + 1, isActive: index == model.guidanceStepIndex)
                                .id(step.id)
                                .onTapGesture {
                                    model.guidanceStepIndex = index
                                    withAnimation(.easeInOut(duration: 0.25)) {
                                        proxy.scrollTo(step.id, anchor: .center)
                                    }
                                }
                        }
                        if steps.count == 1 && model.latestGuidanceSteps.isEmpty {
                            warningBanner
                        }
                    }
                    .padding(16)
                ...
                .onChange(of: model.guidanceStepIndex) { _, newIndex in
                ...
                .onChange(of: steps.count) { _, _ in
                    if model.guidanceStepIndex >= steps.count {
                        model.guidanceStepIndex = max(0, steps.count - 1)
                    }
                }
            }

            // Voice hint
            Text("Say \"Next step\" or \"Previous step\" to move.")
                .font(.system(size: 9))
    ...
    private func guidanceStepCard(step: ChatStep, number: Int, isActive: Bool) -> some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                ...
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .stroke(isActive ? primaryBlue.opacity(0.5) : Color.white.opacity(0.08), lineWidth: isActive ? 2 : 1)
                )
        )
    }

    @ViewBuilder
    private func guidanceDiagramCard(step: ChatStep) -> some View {
        Group {
                ...
                AsyncImage(url: url) { phase in
                    ...
                    case .success(let img):
                        img.resizable()
                            ...
                            .clipped()
                    case .failure:
                        placeholderDiagramView(label: "Image unavailable")
                    case .empty:
                        ProgressView()
                            .frame(maxWidth: .infinity)
                            .frame(height: 140)
                    @unknown default:
                        placeholderDiagramView(label: "FIG \(step.id.hashValue % 100)")
                    }
                }
            } else {
                placeholderDiagramView(label: "FIG \(step.id.hashValue % 100)")
    ...
    private func placeholderDiagramView(label: String) -> some View {
        ZStack {
    ...
    private var warningBanner: some View {
        HStack(alignment: .top, spacing: 10) {
            ...
            Text("Do not move the patient unless there is immediate danger.")
                .font(.system(size: 11))
                .foregroundStyle(Color(white: 0.9).opacity(0.9))
        }
        .padding(10)
...
// MARK: - Right top: Assistant Chat panel
struct AssistantChatPanelView: View {
    @Environment(CrisisCopilotModel.self) private var model
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            HStack {
                HStack(spacing: 12) {
                    RoundedRectangle(cornerRadius: 8)
                        .fill(
                            LinearGradient(
                                colors: [primaryBlue, Color.cyan.opacity(0.8)],
                                startPoint: .topLeading,
                                endPoint: .bottomTrailing
                            )
                        )
            ...
            .background(
                LinearGradient(
                    colors: [primaryBlue.opacity(0.2), Color.clear],
                    startPoint: .leading,
                    endPoint: .trailing
                )
            )
            ...
            ScrollViewReader { proxy in
                ScrollView {
                    LazyVStack(alignment: .leading, spacing: 12) {
                        Text("Today, \(model.sessionStartTime?.formatted(date: .abbreviated, time: .shortened) ?? "—")")
                            .font(.system(size: 10))
                            .foregroundStyle(.secondary)
            ...
            // SPEAK TO AI button
            Button(action: { orbModel.handleTap() }) {
                HStack(spacing: 12) {
                    ...
                    Spacer()
                    if orbModel.orbState == .listening {
                        Circle()
                            .fill(Color.red)
        ...
        .onChange(of: orbModel.orbState) { _, newState in
            ...
            Task { @MainActor in
                ...
                triggerOrbAnimations(for: newState)
            }
        }
        .onAppear { triggerOrbAnimations(for: .idle) }
    ...
    private func triggerOrbAnimations(for state: VoiceOrbModel.OrbState) {
        // Minimal orb state handling if we rely on SPEAK TO AI button for primary affordance
    }
}

// MARK: - Menu panel (scenario, session controls)
struct MenuPanelView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            HStack(spacing: 8) {
                Menu {
                    ForEach(CrisisScenario.allCases, id: \.self) { scenario in
...
// MARK: - Misc panel (Alert EMS, Light, Siren) — panel 4
struct MiscActionsPanelView: View {
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            HStack(spacing: 0) {
                miscButton(icon: "phone.and.waveform.fill", label: "Alert EMS", color: .red)
            Rectangle().fill(Color.white.opacity(0.1)).frame(width: 1, height: 32)
            miscButton(icon: "flashlight.on.fill", label: "Light", color: .yellow)
            Rectangle().fill(Color.white.opacity(0.1)).frame(width: 1, height: 32)
            miscButton(icon: "speaker.wave.2.fill", label: "Siren", color: .white)
            }
        }
        .padding(8)
        .modifier(GlassPanelModifier())
    ...
    private func miscButton(icon: String, label: String, color: Color) -> some View {
        Group {
            if label == "Alert EMS" {
                Link(destination: URL(string: "tel:911")!) {
                    miscButtonContent(icon: icon, label: label, color: color)
                }
                .buttonStyle(.plain)
            } else {
                Button(action: {}) {
    ...
    private func miscButtonContent(icon: String, label: String, color: Color) -> some View {
        VStack(spacing: 4) {
