...
class CrisisCopilotModel {
    ...
    /// Guidance panel: which step is in view / selected (0-based). Voice or scroll can change this.
    var guidanceStepIndex: Int = 0
    ...
    /// Steps from the most recent assistant message that has steps (for Guidance panel).
    var latestGuidanceSteps: [ChatStep] {
        messages.last(where: { $0.role == .assistant && !$0.steps.isEmpty })?.steps ?? []
    }

    /// Move Guidance panel to next step (voice or UI). No-op if already at last.
    func guidanceGoNext() {
        let steps = latestGuidanceSteps
        guard !steps.isEmpty else { return }
        if guidanceStepIndex < steps.count - 1 {
            guidanceStepIndex += 1
        }
    }

    /// Move Guidance panel to previous step. No-op if already at first.
    func guidanceGoBack() {
        if guidanceStepIndex > 0 {
            guidanceStepIndex -= 1
        }
    }
}
